...
func main() {
	port := flag.Int("p", 8080, "port listen to")
	flag.Parse()
	...
	err = s.Serve(lis)
	if err != nil {
		logorus.Fatalf("could not serve: %v", err)
	}
}
...
func (server) Say(ctx context.Context, text *pb.Text) (*pb.Speech, error) {
	...
	cmd := exec.Command("flite", "-t", os.Args[1], "-o", f.Name())
	...
	if err != nil {
		return nil, fmt.Errorf("could not ewad tmp file: %v", err)
	}
